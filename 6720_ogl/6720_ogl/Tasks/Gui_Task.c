...
extern bool handle_OUT_ONOFF_hotkey(uint32_t key, lv_event_t *e)
{
    ...
    if (OUT_ONOFF_led_on) {
      if(current_settings.output_status){
        current_settings.output_hardware_state = 1;  // Output is ON
      }
    } else {
        current_settings.output_hardware_state = 0;  // Output is OFF
    }  
    
      // Save the output state to EEPROM
    save_settings_to_eeprom(&current_settings);
