...
//***********************封装函数*************************************
// 封装通用的事件回调和样式添加函数
void add_event_and_style_generic(lv_obj_t *obj, lv_event_cb_t event_cb, lv_group_t *group, bool *in_group_flag, bool *cb_added_flag, lv_style_t *style) {
    if (!obj || !event_cb || !group || !style) return;

    // 确保事件回调只添加一次
    if (!(*cb_added_flag)) {
        lv_obj_add_event_cb(obj, event_cb, LV_EVENT_KEY, NULL);  // 添加事件回调
        lv_obj_add_style(obj, style, LV_PART_MAIN | LV_STATE_FOCUSED);  // 添加样式
        *cb_added_flag = true;  // 设置标志，避免重复添加
    }

    // 确保控件在 group 中
    if (!(*in_group_flag)) {
        lv_group_add_obj(group, obj);  // 将控件添加到 group 中
        *in_group_flag = true;  // 设置标志，避免重复添加
    }
        // 设置焦点并进入编辑模式
    lv_group_focus_obj(obj);        // 焦点给控件
    lv_group_set_editing(group, true);  // 进入编辑态
}
// 封装退出编辑模式的操作
void exit_edit_mode(lv_group_t *group, lv_obj_t *obj, bool *in_group_flag, lv_obj_t *focus_obj) {
    if (group) lv_group_set_editing(group, false);  // 退出编辑态
...
static void s2_enter_beep_edit(lv_group_t *g) {
    ...
    // 在进入编辑模式之前，确保退出之前的编辑模式
    //exit_edit_mode(g, s2_beep_val, &s2_beep_in_group, s2_btns[0]);
    // 重新添加事件回调和样式
    add_event_and_style_generic(s2_beep_val, s2_beep_edit_key_cb, g, &s2_beep_in_group, &s2_beep_cb_added, &s2_focus_style);
    s2_beep_refresh();  // 刷新标签文本，确保显示正确的蜂鸣器状态
}
...
// 背光亮度的最小值、最大值和步进值
static const int16_t s2_light_min = 5, s2_light_max = 100, s2_light_step = 1;


static void s2_light_refresh(void) {
    if (!s2_light_val) return;
    // 确保亮度在合理范围内
    if (current_settings.backlight_brightness < s2_light_min) {
        current_settings.backlight_brightness = s2_light_min;
    }
    if (current_settings.backlight_brightness > s2_light_max) {
        current_settings.backlight_brightness = s2_light_max;
    }
    // 更新标签文本，显示当前亮度值
    lv_label_set_text_fmt(s2_light_val, "%d%%", current_settings.backlight_brightness);
    printf("Backlight brightness updated: %d%%\n", current_settings.backlight_brightness);  // 打印亮度

    // 同步到硬件
    SetBackLight(current_settings.backlight_brightness);
}
// 背光亮度编辑时的按键回调
static void s2_light_edit_key_cb(lv_event_t *e) {
    ...
    // 如果按下回车或编码器，退出编辑模式
    if (key == LV_KEY_ENTER || key == LV_KEY_ENCODER) {
        exit_edit_mode(g, s2_light_val, &s2_light_in_group, s2_btns[1]);
        return;
    }

    // 如果按下菜单键，返回首页
    if (key == LV_KEY_MENU) {
        exit_edit_mode(g, s2_light_val, &s2_light_in_group, NULL);
        show_page(PAGE_HOME);
        return;
    }
    ...
    if (key == LV_KEY_UP || key == LV_KEY_DOWN) {
        // 上下键调整亮度
        if (key == LV_KEY_UP) {
            current_settings.backlight_brightness += s2_light_step;
...
static void s2_enter_light_edit(lv_group_t *g) {
    ...
    // 使用封装函数添加事件回调和样式，并加入到 group 中
    add_event_and_style_generic(s2_light_val, s2_light_edit_key_cb, g, &s2_light_in_group, &s2_light_cb_added, &s2_focus_style);
    // 刷新标签文本，确保显示正确的亮度
    s2_light_refresh();
...
static void s2_mem_refresh(void) {
    ...
    // 确保值在合理范围内
    if (current_settings.storage_group < s2_mem_min) current_settings.storage_group = s2_mem_min;
    if (current_settings.storage_group > s2_mem_max) current_settings.storage_group = s2_mem_max;

    // 更新标签文本，显示当前存储组值
    lv_label_set_text_fmt(label_mem_num, "%03d", current_settings.storage_group); 
    lv_label_set_text_fmt(s2_mem_val, "%03d", current_settings.storage_group);
    printf("Storage group updated: %d\n", current_settings.storage_group);  // 打印存储组值
}
...
static void s2_mem_edit_key_cb(lv_event_t *e) {
    ...
    // 如果按下回车或编码器，退出编辑模式
    if (key == LV_KEY_ENTER || key == LV_KEY_ENCODER) {
        exit_edit_mode(g, s2_mem_val, &s2_mem_in_group, s2_btns[2]);
        return;
    }

    // 如果按下菜单键，返回首页
    if (key == LV_KEY_MENU) {
        exit_edit_mode(g, s2_mem_val, &s2_mem_in_group, NULL);
        show_page(PAGE_HOME);
        return;
    }
    ...
    if (key == LV_KEY_UP || key == LV_KEY_DOWN) {
        ...
        if (key == LV_KEY_UP) {
            // 增加存储组值
            if (current_settings.storage_group < s2_mem_max) {
                current_settings.storage_group += s2_mem_step;  // 增加值
            }
        } else if (key == LV_KEY_DOWN) {
            // 减少存储组值
            if (current_settings.storage_group > s2_mem_min) {
                current_settings.storage_group -= s2_mem_step;  // 减少值
            }
        }

        // 更新 current_settings.storage_group，并保存到 EEPROM
        save_settings_to_eeprom(&current_settings);  // 保存设置到 EEPROM
...
static void s2_enter_mem_edit(lv_group_t *g) {
    ...
    // 使用封装函数添加事件回调和样式，并加入到 group 中
    add_event_and_style_generic(s2_mem_val, s2_mem_edit_key_cb, g, &s2_mem_in_group, &s2_mem_cb_added, &s2_focus_style);
    // 刷新标签文本，确保显示正确的存储组值
    s2_mem_refresh();
...
static inline void s2_set_topbar_keep_icon(bool on) {
    if (guider_ui.screen_1_label_KEEP) {
        if (on) {
            // 显示图标
            lv_obj_clear_flag(guider_ui.screen_1_label_KEEP, LV_OBJ_FLAG_HIDDEN);  // 清除隐藏标志
        } else {
            // 隐藏图标
            lv_obj_add_flag(guider_ui.screen_1_label_KEEP, LV_OBJ_FLAG_HIDDEN);  // 添加隐藏标志
        }
    }
}
...
static void s2_ato_refresh(void){
    ...
    // 从结构体获取当前输出状态
    lv_label_set_text(s2_ato_val, current_settings.output_status ? "ON" : "OFF");  // 显示 "ON"/"OFF"
    s2_set_topbar_keep_icon(current_settings.output_status);  // 更新顶部图标
    printf("output_status updated: %d\n", current_settings.output_status);  // 打印存储组值
  
  if(!current_settings.output_status)
  {  
  current_settings.output_hardware_state = 0;
  }
  
  if (current_settings.output_hardware_state == 1) {
        OUT_ONOFF_led_on = 1;  // 恢复输出状态为 ON
    } else {
        OUT_ONOFF_led_on = 0;  // 恢复输出状态为 OFF
    }  
...
/* 在 AtoVal 上的按键处理：UP/DOWN 切换文字；ENTER 退出编辑；MENU 回首页 */
static void s2_ato_edit_key_cb(lv_event_t *e) {
    ...
    // 如果按下回车或编码器，退出编辑模式
    if (key == LV_KEY_ENTER || key == LV_KEY_ENCODER) {
        exit_edit_mode(g, s2_ato_val, &s2_ato_in_group, s2_btns[3]);
        return;
    }

    // 如果按下菜单键，返回首页
    if (key == LV_KEY_MENU) {
        exit_edit_mode(g, s2_ato_val, &s2_ato_in_group, NULL);
        show_page(PAGE_HOME);
        return;
    }
    ...
    if (key == LV_KEY_UP || key == LV_KEY_DOWN) {
        // 上下键切换输出状态
        current_settings.output_status = !current_settings.output_status;  // 切换状态
...
static void s2_enter_ato_edit(lv_group_t *g) {
    ...
    add_event_and_style_generic(s2_ato_val, s2_ato_edit_key_cb, g, &s2_ato_in_group, &s2_ato_cb_added, &s2_focus_style);
    s2_ato_refresh();  // 刷新标签文本，确保显示正确的输出状态  
}
