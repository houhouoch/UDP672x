...
#include "LinkList.h"
static LinkList_Def *BatTesterFile_listHead = NULL;
static FRESULT FATFS_FindFile_Recursive(const char *device, const char *path, const char *fileName)
{
    return FATFS_find(BatTesterFile_listHead, device, path, fileName);
}
...
{
    lv_obj_t *backg = ui.BatTesterManager_cont;
...
static void __Table_setFocus(uint16_t *row_y)
{
    ...
    lv_obj_set_size(ui.BatTesterManager_img_back, width, height);
    lv_obj_set_pos(ui.BatTesterManager_img_back, x, y);
    lv_obj_set_hidden(ui.BatTesterManager_img_back, 0);
}
...
static void __Tabel_Refresh_Row(uint16_t baseIndex, uint16_t i)
{
    ...
    const LinkList_Def *pnode = LinkList_At(BatTesterFile_listHead, i);
    if(pnode == NULL) {
        return ;
    }
...
{
    LinkList_Def *current = LinkList_At(BatTesterFile_listHead, disp_baseIndex + edit_row_y - MIN_ROW);
    if(current != NULL) {
        return current->path;
    } else {
        return "";
...
void BatTester_Page_Manager_Table_handler(lv_obj_t *obj, lv_event_t event)
{
    ...
    if(lv_obj_is_visible(ui.InputWidget_cont_inputString)) {
        ...
        if(event == LV_EVENT_KEY)
        {
            ...
            sprintf(fileName_temp, "%s.batt.csv", fileName);
            uint8_t result = 1;
            switch(key) {
                case LV_KEY_SAVE_FLASH: {
                    result = LinkList_Find_duplicate(BatTesterFile_listHead, "FLASH", fileName_temp);
                }
                break;
                case LV_KEY_SAVE_UDISK: {
                    result = LinkList_Find_duplicate(BatTesterFile_listHead, "USB", fileName_temp);
                }
                break;
                case LV_KEY_ENTER: {
            ...
            if(result == 0) {
                ...
                if(key == LV_KEY_SAVE_FLASH) {
                    //另存为到U盘
                    sprintf(savefileName, "%s/%s.batt.csv", BAT_TESTER_FILE_FLASH_PATH, fileName);
                    if(0 != BatTester_Save(&BatTester, savefileName)) {
                        IF_MsgBox_Tips_Mult(&mStr_Save_Tips);
                    }
                } else
                    if(key == LV_KEY_SAVE_UDISK) {
                        //另存为到USB
                        sprintf(savefileName, "%s/%s.batt.csv", BAT_TESTER_FILE_USB_PATH, fileName);
                        if(0 != BatTester_Save(&BatTester, savefileName)) {
                            IF_MsgBox_Tips_Mult(&mStr_Save_Tips);
                        }
                    } else
                        if(key == LV_KEY_ENTER) {
                            ...
                            if(current != NULL) {
                                if(0 == strcmp("FLASH", current->device)) {
                                    sprintf(savefileName, "%s/%s.batt.csv", BAT_TESTER_FILE_FLASH_PATH, fileName);
                                    if(0 != BatTester_Rename(current->path, savefileName)) {
                                        BatTester_Delete(current->path);
                                    }
                                } else
                                    if(0 == strcmp("USB", current->device)) {
                                        sprintf(savefileName, "%s/%s.batt.csv", BAT_TESTER_FILE_USB_PATH, fileName);
            ...
            lv_event_send_refresh_recursive(ui.BatTesterManager_cont);
        }
        return;
    }
    ...
    switch(event)
    {
        case LV_EVENT_KEY:
        {
            switch(key)
            {
                case LV_KEY_SAVE: {
                    if(itemCount > 0) {
                        BatTester_Save(&BatTester, __GetFocusFileName());
                        printf("Save File:|%s|\r\n", __GetFocusFileName());
                        refreshFlag = 1;
                        IF_MsgBox_Tips_Mult(&mStr_Save_Tips);
                    }
                ...
                case LV_KEY_LOAD: {
                    if(itemCount > 0) {
                        memset(&BatTester.points, 0, sizeof(BatTester.points));
                        BatTester_Load(&BatTester, __GetFocusFileName());
                        printf("Load File:|%s|\r\n", __GetFocusFileName());
                        refreshFlag = 1;
                ...
                break;
                case LV_KEY_DELETE: {
                    BatTester_Delete(__GetFocusFileName());
                    refreshFlag = 1;
    ...
    if(refreshFlag == 1) {
        LinkList_DeleteAll(BatTesterFile_listHead);
        FATFS_FindFile_Recursive("FLASH", BAT_TESTER_FILE_FLASH_PATH, "*.batt.csv");
        FATFS_FindFile_Recursive("USB", BAT_TESTER_FILE_USB_PATH, "*.batt.csv");
        itemCount = LinkList_Count(BatTesterFile_listHead);
...
/*
 * @brief       void BatTester_Page_Manager_Enter(void)
 * @details     进入BatTester管理页面
 */
void BatTester_Page_Manager_Enter(void)
{
    lv_obj_set_hidden(ui.BatTesterManager_cont, 0);
    disp_baseIndex = 0;
...
{
    LinkList_DeleteAll(BatTesterFile_listHead);
    lv_obj_set_hidden(ui.BatTesterManager_cont, 1);
}
...
{
    {
        setup_scr_BatTesterManager(&ui);
        lv_obj_set_parent(ui.BatTesterManager_cont, ui.Back_cont_back);
    }
    __Table_Init();
    BatTesterFile_listHead = LinkList_Create("", "", "");
    BatTester_Page_Manager_Enter();
    BatTester_Page_Manager_Exit();
}
