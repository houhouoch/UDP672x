...
#include "LinkList.h"
static LinkList_Def *DelayFile_listHead = NULL;
...
{
    return FATFS_find(DelayFile_listHead, device, path, fileName);
}
...
static void __Table_Init(void)
{
    lv_obj_t *backg = ui.DelayManagePage_cont;
...
static void __Table_setFocus(uint16_t *row_y)
{
    ...
    lv_obj_set_size(ui.DelayManagePage_img_back, width, height);
    lv_obj_set_pos(ui.DelayManagePage_img_back, x, y);
    lv_obj_set_hidden(ui.DelayManagePage_img_back, 0);
}
...
static void __Tabel_Refresh_Row(uint16_t baseIndex, uint16_t i)
{
    ...
    const LinkList_Def *pnode = LinkList_At(DelayFile_listHead, i);
    if(pnode == NULL) {
        return ;
    }
...
{
    LinkList_Def *current = LinkList_At(DelayFile_listHead, disp_baseIndex + edit_row_y - MIN_ROW);
    if(current != NULL) {
        return current->path;
    } else {
        return "";
...
void DelayPage_Manager_Table_handler(lv_obj_t *obj, lv_event_t event)
{
    ...
    if(lv_obj_is_visible(ui.InputWidget_cont_inputString)) {
        ...
        if(event == LV_EVENT_KEY)
        {
            ...
            sprintf(fileName_temp, "%s.dly.csv", fileName);
            uint8_t result = 1;
            switch(key) {
                case LV_KEY_SAVE_FLASH: {
                    result = LinkList_Find_duplicate(DelayFile_listHead, "FLASH", fileName_temp);
                }
                break;
                case LV_KEY_SAVE_UDISK: {
                    result = LinkList_Find_duplicate(DelayFile_listHead, "USB", fileName_temp);
                }
                break;
                case LV_KEY_ENTER: {
            ...
            if(result == 0) {
                ...
                if(key == LV_KEY_SAVE_FLASH) {
                    //另存为到U盘
                    sprintf(savefileName, "%s/%s.dly.csv", DELAYOUT_FILE_FLASH_PATH, fileName);
                    if(0 != DelayOut_Save(&delayoutInfo, savefileName)) {
                        IF_MsgBox_Tips_Mult(&mStr_Save_Tips);
                    }
                } else
                    if(key == LV_KEY_SAVE_UDISK) {
                        //另存为到USB
                        sprintf(savefileName, "%s/%s.dly.csv", DELAYOUT_FILE_USB_PATH, fileName);
                        if(0 != DelayOut_Save(&delayoutInfo, savefileName)) {
                            IF_MsgBox_Tips_Mult(&mStr_Save_Tips);
                        }
                    } else
                        if(key == LV_KEY_ENTER) {
                            ...
                            if(current != NULL) {
                                if(0 == strcmp("FLASH", current->device)) {
                                    sprintf(savefileName, "%s/%s.dly.csv", DELAYOUT_FILE_FLASH_PATH, fileName);
                                    if(0 != DelayOut_Rename(current->path, savefileName)) {
                                        DelayOut_Delete(current->path);
                                    }
                                } else
                                    if(0 == strcmp("USB", current->device)) {
                                        sprintf(savefileName, "%s/%s.dly.csv", DELAYOUT_FILE_USB_PATH, fileName);
            ...
            lv_event_send_refresh_recursive(ui.DelayManagePage_cont);
        }
        return;
    }
    ...
    switch(event)
    {
        case LV_EVENT_KEY:
        {
            switch(key)
            {
                case LV_KEY_SAVE: {
                    if(itemCount > 0) {
                        DelayOut_Save(&delayoutInfo, __GetFocusFileName());
                        printf("Save File:|%s|\r\n", __GetFocusFileName());
                        refreshFlag = 1;
                        IF_MsgBox_Tips_Mult(&mStr_Save_Tips);
                    }
                ...
                case LV_KEY_LOAD: {
                    if(itemCount > 0) {
                        memset(&delayoutInfo.points, 0, sizeof(delayoutInfo.points));
                        DelayOut_Load(&delayoutInfo, __GetFocusFileName());
                        printf("Load File:|%s|\r\n", __GetFocusFileName());
                        refreshFlag = 1;
                ...
                break;
                case LV_KEY_DELETE: {
                    DelayOut_Delete(__GetFocusFileName());
                    refreshFlag = 1;
    ...
    if(refreshFlag == 1) {
        LinkList_DeleteAll(DelayFile_listHead);
        FATFS_FindFile_Recursive("FLASH", DELAYOUT_FILE_FLASH_PATH, "*.dly.csv");
        FATFS_FindFile_Recursive("USB", DELAYOUT_FILE_USB_PATH, "*.dly.csv");
        itemCount = LinkList_Count(DelayFile_listHead);
...
{
    lv_obj_set_hidden(ui.DelayManagePage_cont, 0);
    disp_baseIndex = 0;
...
{
    LinkList_DeleteAll(DelayFile_listHead);
    lv_obj_set_hidden(ui.DelayManagePage_cont, 1);
}
...
{
    {
        setup_scr_DelayManagePage(&ui);
        lv_obj_set_parent(ui.DelayManagePage_cont, ui.Back_cont_back);
    }
    __Table_Init();
    //
    DelayFile_listHead = LinkList_Create("", "", "");
    DelayPage_Manager_Enter();
    DelayPage_Manager_Exit();
}
